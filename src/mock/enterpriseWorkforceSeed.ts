import type { AttendanceGroup, Department, Employee, Team } from '@/types'
import { createDefaultFreePunchConfig, createDefaultPricingConfig } from '@/constants/attendanceGroupPricing'
import { getDefaultScheduleRuleForSeed } from '@/services/scheduleGroup'
import { ensureGroupVersions } from '@/services/attendanceGroupVersion'

const ENT_STARS = 'ent_stars_telecom'
const ENT_CM = 'ent_china_mobile_agent'
const ENT_PINGAN = 'ent_pingan_partner'
const ENT_SH = 'ent_china_telecom_agent'

const defaultCompliance = {
  enabled: false,
  maxDailyHours: 12,
  maxWeeklyHours: 60,
  minShiftIntervalHours: 12,
  maxMonthlyHours: 260,
  maxConsecutiveWorkdays: 3,
}

/** 各企业组织架构（含根节点与待分配） */
export const seedMultiEnterpriseDepartments: Department[] = [
  {
    id: 'dept_root_ent_china_mobile_agent',
    name: '中国移动朝阳企业公司',
    parentId: null,
    sort: 0,
    enterpriseId: ENT_CM,
    orgType: 'enterprise',
    nodeType: 'branch',
  },
  {
    id: 'dept_unassigned_ent_china_mobile_agent',
    name: '待入驻人员',
    parentId: null,
    sort: 1,
    enterpriseId: ENT_CM,
    nodeType: 'leaf',
    description: '用于管理待入职及尚未分配部门和岗位的人员',
  },
  {
    id: 'dept_cm_hr',
    name: '人事行政部',
    parentId: 'dept_root_ent_china_mobile_agent',
    sort: 1,
    enterpriseId: ENT_CM,
    nodeType: 'leaf',
    managerEmployeeId: 'emp_002',
    managerEmployeeIds: ['emp_002'],
  },
  {
    id: 'dept_cm_field',
    name: '外勤推广部',
    parentId: 'dept_root_ent_china_mobile_agent',
    sort: 3,
    enterpriseId: ENT_CM,
    nodeType: 'leaf',
    attendanceGroupId: 'ag_sales',
    managerEmployeeId: 'emp_005',
    managerEmployeeIds: ['emp_005'],
    description: '负责5G套餐推广、套餐升档等外勤任务',
  },
  {
    id: 'dept_root_ent_pingan_partner',
    name: '中国移动浙江企业公司',
    parentId: null,
    sort: 0,
    enterpriseId: ENT_PINGAN,
    orgType: 'enterprise',
    nodeType: 'branch',
  },
  {
    id: 'dept_unassigned_ent_pingan_partner',
    name: '待入驻人员',
    parentId: null,
    sort: 1,
    enterpriseId: ENT_PINGAN,
    nodeType: 'leaf',
    description: '用于管理待入职及尚未分配部门和岗位的人员',
  },
  {
    id: 'dept_pj_store',
    name: '营业厅运营部',
    parentId: 'dept_root_ent_pingan_partner',
    sort: 1,
    enterpriseId: ENT_PINGAN,
    nodeType: 'branch',
    attendanceGroupId: 'ag_pj_store',
    managerEmployeeId: 'emp_006',
    managerEmployeeIds: ['emp_006'],
    description: '浙江区域终端陈列与推广',
  },
  {
    id: 'dept_pj_field',
    name: '渠道推广组',
    parentId: 'dept_pj_store',
    sort: 1,
    enterpriseId: ENT_PINGAN,
    nodeType: 'leaf',
    attendanceGroupId: 'ag_pj_store',
  },
  {
    id: 'dept_root_ent_china_telecom_agent',
    name: '中国移动上海企业公司',
    parentId: null,
    sort: 0,
    enterpriseId: ENT_SH,
    orgType: 'enterprise',
    nodeType: 'branch',
  },
  {
    id: 'dept_unassigned_ent_china_telecom_agent',
    name: '待入驻人员',
    parentId: null,
    sort: 1,
    enterpriseId: ENT_SH,
    nodeType: 'leaf',
    description: '用于管理待入职及尚未分配部门和岗位的人员',
  },
  {
    id: 'dept_sh_hall',
    name: '营业厅运营部',
    parentId: 'dept_root_ent_china_telecom_agent',
    sort: 1,
    enterpriseId: ENT_SH,
    nodeType: 'leaf',
    attendanceGroupId: 'ag_sh_hall',
    managerEmployeeId: 'emp_sh_001',
    managerEmployeeIds: ['emp_sh_001'],
    description: '商圈营业厅引流与业务办理',
  },
]

/** 补充各企业灵工人员 */
export const seedMultiEnterpriseEmployees: Employee[] = [
  {
    id: 'emp_pj_001',
    name: '孙丽',
    employeeNo: 'E2001',
    departmentId: 'dept_pj_field',
    enterpriseId: ENT_PINGAN,
    position: '推广专员',
    hireDate: '2023-04-12',
    skills: ['陈列检查'],
    preferredShiftIds: [],
    unavailableDates: [],
    status: 'active',
    phone: '13800002001',
    gender: 'female',
    realNameVerified: true,
  },
  {
    id: 'emp_pj_002',
    name: '马强',
    employeeNo: 'E2002',
    departmentId: 'dept_pj_field',
    enterpriseId: ENT_PINGAN,
    position: '班组长',
    hireDate: '2022-08-01',
    skills: ['安全管理'],
    preferredShiftIds: [],
    unavailableDates: [],
    status: 'active',
    phone: '13800002002',
    realNameVerified: true,
  },
  {
    id: 'emp_pj_pending',
    name: '周待入职',
    employeeNo: 'E2901',
    departmentId: 'dept_unassigned_ent_pingan_partner',
    enterpriseId: ENT_PINGAN,
    position: '待入驻',
    hireDate: '2026-07-30',
    skills: [],
    preferredShiftIds: [],
    unavailableDates: [],
    status: 'pending',
    dataSource: 'recruit',
    phone: '13800002901',
    realNameVerified: false,
  },
  {
    id: 'emp_sh_001',
    name: '黄明',
    employeeNo: 'E3001',
    departmentId: 'dept_sh_hall',
    enterpriseId: ENT_SH,
    position: '营业厅引导员',
    hireDate: '2024-01-15',
    skills: [],
    preferredShiftIds: ['shift_morning'],
    unavailableDates: [],
    status: 'active',
    phone: '13800003001',
    gender: 'male',
    realNameVerified: true,
  },
  {
    id: 'emp_sh_002',
    name: '徐芳',
    employeeNo: 'E3002',
    departmentId: 'dept_sh_hall',
    enterpriseId: ENT_SH,
    position: '推广专员',
    hireDate: '2024-06-20',
    skills: [],
    preferredShiftIds: ['shift_afternoon'],
    unavailableDates: [],
    status: 'active',
    phone: '13800003002',
    gender: 'female',
    realNameVerified: true,
  },
  {
    id: 'emp_stars_001',
    name: '林浩',
    employeeNo: 'E0101',
    departmentId: 'dept_hr',
    enterpriseId: ENT_STARS,
    position: 'HR专员',
    hireDate: '2021-03-01',
    skills: [],
    preferredShiftIds: ['shift_morning'],
    unavailableDates: [],
    status: 'active',
    phone: '13800000101',
    realNameVerified: true,
  },
  {
    id: 'emp_stars_002',
    name: '赵婷',
    employeeNo: 'E0102',
    departmentId: 'dept_hr',
    enterpriseId: ENT_STARS,
    position: '行政助理',
    hireDate: '2022-05-18',
    skills: [],
    preferredShiftIds: ['shift_morning'],
    unavailableDates: [],
    status: 'active',
    phone: '13800000102',
    realNameVerified: true,
  },
  /** —— 抢班人员池（演示） —— */
  {
    id: 'emp_grab_cm_001',
    name: '何晓燕',
    employeeNo: 'G1101',
    departmentId: 'dept_prod_a',
    enterpriseId: ENT_CM,
    position: '营业厅营业员',
    hireDate: '2026-06-12',
    skills: ['健康证'],
    preferredShiftIds: [],
    unavailableDates: [],
    status: 'active',
    phone: '13900001101',
    gender: 'female',
    age: 28,
    idCardNo: '110105199802021234',
    personnelCategory: 'grab',
    realNameVerified: true,
  },
  {
    id: 'emp_grab_cm_002',
    name: '曹俊',
    employeeNo: 'G1102',
    departmentId: 'dept_prod_a',
    enterpriseId: ENT_CM,
    position: '营业厅营业员',
    hireDate: '2026-05-08',
    skills: ['健康证'],
    preferredShiftIds: [],
    unavailableDates: [],
    status: 'active',
    phone: '13900001102',
    gender: 'male',
    age: 29,
    idCardNo: '110105199705153456',
    personnelCategory: 'grab',
    realNameVerified: true,
  },
  {
    id: 'emp_grab_cm_003',
    name: '邓丽',
    employeeNo: 'G1103',
    departmentId: 'dept_prod_a',
    enterpriseId: ENT_CM,
    position: '营业厅收银',
    hireDate: '2026-04-20',
    skills: ['健康证'],
    preferredShiftIds: [],
    unavailableDates: [],
    status: 'active',
    phone: '13900001103',
    gender: 'female',
    age: 25,
    idCardNo: '110105200103087890',
    personnelCategory: 'grab',
    realNameVerified: true,
  },
  {
    id: 'emp_grab_cm_004',
    name: '方磊',
    employeeNo: 'G1104',
    departmentId: 'dept_cm_field',
    enterpriseId: ENT_CM,
    position: '推广专员',
    hireDate: '2026-03-15',
    skills: ['健康证'],
    preferredShiftIds: [],
    unavailableDates: [],
    status: 'active',
    phone: '13900001104',
    gender: 'male',
    age: 27,
    idCardNo: '110105199901019012',
    personnelCategory: 'grab',
    realNameVerified: true,
  },
  {
    id: 'emp_grab_cm_005',
    name: '高敏',
    employeeNo: 'G1105',
    departmentId: 'dept_cm_field',
    enterpriseId: ENT_CM,
    position: '推广专员',
    hireDate: '2026-02-28',
    skills: [],
    preferredShiftIds: [],
    unavailableDates: [],
    status: 'active',
    phone: '13900001105',
    gender: 'female',
    age: 24,
    idCardNo: '110105200204045678',
    personnelCategory: 'grab',
    realNameVerified: true,
  },
  {
    id: 'emp_grab_stars_001',
    name: '黄涛',
    employeeNo: 'G2101',
    departmentId: 'dept_logistics',
    enterpriseId: ENT_STARS,
    position: '理货员',
    hireDate: '2026-05-01',
    skills: ['叉车证'],
    preferredShiftIds: [],
    unavailableDates: [],
    status: 'active',
    phone: '13900002101',
    gender: 'male',
    age: 31,
    idCardNo: '330106199412127890',
    personnelCategory: 'grab',
    realNameVerified: true,
  },
  {
    id: 'emp_grab_stars_002',
    name: '蒋雪',
    employeeNo: 'G2102',
    departmentId: 'dept_logistics',
    enterpriseId: ENT_STARS,
    position: '装卸工',
    hireDate: '2026-06-18',
    skills: [],
    preferredShiftIds: [],
    unavailableDates: [],
    status: 'active',
    phone: '13900002102',
    gender: 'female',
    age: 28,
    idCardNo: '330106199808081234',
    personnelCategory: 'grab',
    realNameVerified: true,
  },
  {
    id: 'emp_grab_stars_003',
    name: '罗斌',
    employeeNo: 'G2103',
    departmentId: 'dept_logistics',
    enterpriseId: ENT_STARS,
    position: '配送司机',
    hireDate: '2026-01-10',
    resignDate: '2026-07-12',
    skills: ['驾驶证'],
    preferredShiftIds: [],
    unavailableDates: [],
    status: 'resigned',
    phone: '13900002103',
    gender: 'male',
    age: 35,
    idCardNo: '330106199105156789',
    personnelCategory: 'grab',
    realNameVerified: true,
  },
  {
    id: 'emp_grab_pj_001',
    name: '谢芳',
    employeeNo: 'G3101',
    departmentId: 'dept_pj_field',
    enterpriseId: ENT_PINGAN,
    position: '终端销售员',
    hireDate: '2026-04-02',
    resignDate: '2026-07-20',
    skills: ['健康证'],
    preferredShiftIds: [],
    unavailableDates: [],
    status: 'resigned',
    phone: '13900003101',
    gender: 'female',
    age: 30,
    idCardNo: '330102199608082345',
    personnelCategory: 'grab',
    realNameVerified: true,
  },
  {
    id: 'emp_grab_pj_002',
    name: '韩伟',
    employeeNo: 'G3102',
    departmentId: 'dept_pj_field',
    enterpriseId: ENT_PINGAN,
    position: '推广专员',
    hireDate: '2026-05-22',
    skills: [],
    preferredShiftIds: [],
    unavailableDates: [],
    status: 'active',
    phone: '13900003102',
    gender: 'male',
    age: 26,
    idCardNo: '330102200001019876',
    personnelCategory: 'grab',
    realNameVerified: true,
  },
]

export const seedMultiEnterpriseTeams: Team[] = [
  {
    id: 'team_pj_am',
    name: '浙江营业厅早班',
    departmentId: 'dept_pj_field',
    attendanceGroupId: 'ag_pj_store',
    memberIds: ['emp_006', 'emp_pj_001'],
    hourlyRate: 42,
    description: '终端陈列检查早班',
  },
  {
    id: 'team_sh_hall',
    name: '上海营业厅推广组',
    departmentId: 'dept_sh_hall',
    attendanceGroupId: 'ag_sh_hall',
    memberIds: ['emp_sh_001', 'emp_sh_002'],
    hourlyRate: 40,
    description: '商圈营业厅引流推广',
  },
  {
    id: 'team_cm_field',
    name: '朝阳外勤推广组',
    departmentId: 'dept_cm_field',
    attendanceGroupId: 'ag_sales',
    memberIds: ['emp_005', 'emp_007'],
    hourlyRate: 35,
    description: '5G套餐推广与套餐升档外勤',
  },
]

const rawExtraGroups = [
  {
    id: 'ag_pj_store',
    code: 'ZJ-ATT-001',
    name: '浙江营业厅考勤组',
    description: '终端陈列推广，自由打卡计工时',
    status: 'enabled' as const,
    attendanceType: 'free' as const,
    shiftTemplates: [],
    freePunchConfig: {
      ...createDefaultFreePunchConfig(),
      startTime: '08:00',
      endTime: '21:00',
      punchCountMode: 'clock_in_out' as const,
    },
    gpsEnabled: true,
    gpsRadiusMeters: 500,
    punchLocations: [
      {
        id: 'loc_pj_hz',
        name: '杭州西湖营业厅',
        province: '浙江省',
        city: '杭州市',
        district: '西湖区',
        address: '文三路128号',
      },
      {
        id: 'loc_pj_nb',
        name: '宁波鄞州营业厅',
        province: '浙江省',
        city: '宁波市',
        district: '鄞州区',
        address: '中山东路188号',
      },
    ],
    wifiEnabled: true,
    wifiName: 'Sinopec-Store',
    qrcodeEnabled: true,
    compliance: { ...defaultCompliance },
    scheduleRule: getDefaultScheduleRuleForSeed({
      maxConsecutiveDays: 5,
      maxDailyHours: 10,
      maxWeeklyHours: 48,
      maxMonthlyHours: 200,
      minRestHours: 10,
      weekendWork: true,
    }),
    departmentBindings: [
      { departmentId: 'dept_pj_field', departmentName: '营业厅运营部', headcount: 8, managerName: '陈静' },
      { departmentId: 'dept_pj_field', departmentName: '渠道推广组', headcount: 5, managerName: '马强' },
    ],
    payRule: {
      baseHourlyRate: 42,
      nightShiftSubsidy: 5,
      nightShiftTimeRange: '22:00-06:00',
      holidaySubsidy: 60,
    },
    pricingConfig: createDefaultPricingConfig(42),
    minMonthlyOnlineHours: 160,
    attendanceArea: '浙江营业厅',
    createdAt: '2026-06-15T08:00:00.000Z',
    updatedAt: '2026-07-18T10:00:00.000Z',
  },
  {
    id: 'ag_sh_hall',
    code: 'SH-ATT-001',
    name: '上海营业厅考勤组',
    description: '营业厅引流推广，排班+GPS打卡',
    status: 'enabled' as const,
    attendanceType: 'shift' as const,
    shiftTemplates: [
      {
        id: 'st_sh_am',
        name: '早班',
        startTime: '09:00',
        endTime: '17:00',
        hasBreakTime: true,
        breakPeriods: [{ start: '12:00', end: '13:00' }],
        breakRule: '12:00-13:00',
        workHours: 7,
        requiredHeadcount: 6,
        weekendRequiredHeadcount: 8,
        holidayRequiredHeadcount: 4,
      },
      {
        id: 'st_sh_pm',
        name: '晚班',
        startTime: '13:00',
        endTime: '21:00',
        hasBreakTime: true,
        breakPeriods: [{ start: '17:30', end: '18:00' }],
        breakRule: '17:30-18:00',
        workHours: 7.5,
        requiredHeadcount: 4,
        weekendRequiredHeadcount: 6,
        holidayRequiredHeadcount: 3,
      },
    ],
    gpsEnabled: true,
    gpsRadiusMeters: 400,
    punchLocations: [
      {
        id: 'loc_sh_ljz',
        name: '陆家嘴营业厅',
        province: '上海市',
        city: '上海市',
        district: '浦东新区',
        address: '陆家嘴环路1000号',
      },
    ],
    wifiEnabled: true,
    wifiName: 'Sinopec-SH-Hall',
    qrcodeEnabled: true,
    compliance: { ...defaultCompliance },
    scheduleRule: getDefaultScheduleRuleForSeed({
      maxConsecutiveDays: 5,
      maxDailyHours: 9,
      maxWeeklyHours: 45,
      maxMonthlyHours: 176,
      minRestHours: 12,
      weekendWork: true,
    }),
    departmentBindings: [
      { departmentId: 'dept_sh_hall', departmentName: '营业厅运营部', headcount: 6, managerName: '黄明' },
    ],
    payRule: {
      baseHourlyRate: 40,
      nightShiftSubsidy: 6,
      nightShiftTimeRange: '22:00-06:00',
      holidaySubsidy: 70,
    },
    pricingConfig: createDefaultPricingConfig(40),
    minMonthlyOnlineHours: 176,
    attendanceArea: '陆家嘴商圈',
    createdAt: '2026-06-20T08:00:00.000Z',
    updatedAt: '2026-07-22T11:00:00.000Z',
  },
]

export const seedMultiEnterpriseAttendanceGroups: AttendanceGroup[] = rawExtraGroups.map((item) =>
  ensureGroupVersions(item as AttendanceGroup),
)

/** 将基础 seed 部门/人员绑定到正确企业（用于 merge） */
export function patchBaseWorkforceEnterpriseIds(
  departments: Department[],
  employees: Employee[],
): { departments: Department[]; employees: Employee[] } {
  const deptPatches: Record<string, Partial<Department>> = {
    dept_root: { enterpriseId: ENT_STARS, name: '星辰通信企业公司' },
    dept_unassigned: { enterpriseId: ENT_STARS },
    dept_hr: { enterpriseId: ENT_STARS },
    dept_prod: { enterpriseId: ENT_STARS, name: '生产运营部' },
    dept_prod_b: { enterpriseId: ENT_STARS },
    dept_logistics: { enterpriseId: ENT_STARS },
    dept_prod_a: {
      enterpriseId: ENT_CM,
      parentId: 'dept_root_ent_china_mobile_agent',
      name: '中国移动朝阳营业厅',
    },
  }

  const empPatches: Record<string, Partial<Employee>> = {
    emp_001: { enterpriseId: ENT_CM, dataSource: 'qr' },
    emp_002: { enterpriseId: ENT_CM, dataSource: 'recruit' },
    emp_003: {
      enterpriseId: ENT_CM,
      personnelCategory: 'grab',
      position: '营业厅营业员',
    },
    emp_004: {
      enterpriseId: ENT_CM,
      personnelCategory: 'grab',
      position: '营业厅收银',
    },
    emp_005: {
      enterpriseId: ENT_CM,
      departmentId: 'dept_cm_field',
      personnelCategory: 'grab',
      position: '推广专员',
    },
    emp_006: { enterpriseId: ENT_PINGAN, departmentId: 'dept_pj_field', position: '推广专员' },
    emp_007: {
      enterpriseId: ENT_CM,
      departmentId: 'dept_cm_field',
      personnelCategory: 'grab',
      position: '推广专员',
    },
    emp_008: { enterpriseId: ENT_STARS, personnelCategory: 'grab', position: '理货员' },
    emp_009: { enterpriseId: ENT_STARS },
    emp_010: {
      enterpriseId: ENT_STARS,
      personnelCategory: 'grab',
      position: '配送司机',
    },
    emp_pending_001: {
      enterpriseId: ENT_CM,
      departmentId: 'dept_unassigned_ent_china_mobile_agent',
      dataSource: 'recruit',
    },
    emp_pending_002: {
      enterpriseId: ENT_CM,
      departmentId: 'dept_unassigned_ent_china_mobile_agent',
      applyDepartmentId: 'dept_cm_field',
      dataSource: 'qr',
    },
  }

  return {
    departments: departments.map((d) => ({ ...d, ...deptPatches[d.id] })),
    employees: employees.map((e) => ({ ...e, ...empPatches[e.id] })),
  }
}

export function mergeWorkforceSeed(
  baseDepartments: Department[],
  baseEmployees: Employee[],
  baseTeams: Team[],
  baseGroups: AttendanceGroup[],
) {
  const patched = patchBaseWorkforceEnterpriseIds(baseDepartments, baseEmployees)
  const existingDeptIds = new Set(patched.departments.map((d) => d.id))
  const existingEmpIds = new Set(patched.employees.map((e) => e.id))
  const existingTeamIds = new Set(baseTeams.map((t) => t.id))
  const existingGroupIds = new Set(baseGroups.map((g) => g.id))

  const departments = [
    ...patched.departments,
    ...seedMultiEnterpriseDepartments.filter((d) => !existingDeptIds.has(d.id)),
  ]
  const employees = [
    ...patched.employees,
    ...seedMultiEnterpriseEmployees.filter((e) => !existingEmpIds.has(e.id)),
  ]
  const teams = [
    ...baseTeams,
    ...seedMultiEnterpriseTeams.filter((t) => !existingTeamIds.has(t.id)),
  ]
  const attendanceGroups = [
    ...baseGroups.map((g) => {
      if (g.id === 'ag_sales') {
        return {
          ...g,
          departmentBindings: [
            { departmentId: 'dept_cm_field', departmentName: '外勤推广部', headcount: 12, managerName: '刘洋' },
          ],
        }
      }
      return g
    }),
    ...seedMultiEnterpriseAttendanceGroups.filter((g) => !existingGroupIds.has(g.id)),
  ]

  return { departments, employees, teams, attendanceGroups }
}
